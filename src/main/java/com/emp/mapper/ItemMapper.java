...
public class ItemMapper {
	...
	public static List<ItemResponseDto> convertEntityListToResponseList(List<Item> entityList) {
		if (entityList == null)
			return null;
	...
	public static List<Item> convertAddRequestListToEntityList(List<AddItemRequestDto> requestList, int categoryId) {
		if (CollectionUtils.isEmpty(requestList)) {
			return null;
		}
		List<Item> responseList = new ArrayList<Item>(requestList.size());
		for (AddItemRequestDto item : requestList) {
			Item entity = convertAddRequestToEntity(item);
